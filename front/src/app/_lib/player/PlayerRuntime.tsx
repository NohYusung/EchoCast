...
import type { VogopangContent, VogopangContentAudioClip, VogopangContentHole } from './vogopangContent.types';
...
function resolveDurationMs(content: VogopangContent): number {
    ...
    const holeMax = Math.max(
        0,
        ...content.tracks.flatMap((track) => track.holes.map((hole) => hole.start_ms + hole.duration_ms))
    );
...
function collectActiveHoles(content: VogopangContent, currentTime: number): VogopangContentHole[] {
    return content.tracks.flatMap((track) =>
        track.holes.filter((hole) => currentTime >= hole.start_ms && currentTime <= hole.start_ms + hole.duration_ms)
    );
}

function collectActiveAudioClips(content: VogopangContent, currentTime: number): VogopangContentAudioClip[] {
...
export function PlayerRuntime({ content, episodeId }: { content: VogopangContent; episodeId: string }) {
    const durationMs = useMemo(() => resolveDurationMs(content), [content]);
    const [currentTime, setCurrentTime] = useState(0);
    const [isPlaying, setIsPlaying] = useState(false);
    const activeImageIndex = useMemo(() => findActiveImageIndex(content, currentTime), [content, currentTime]);
    const activeHoles = useMemo(() => collectActiveHoles(content, currentTime), [content, currentTime]);
    const activeAudioClips = useMemo(() => collectActiveAudioClips(content, currentTime), [content, currentTime]);
    ...
    return (
        <section className="player-runtime">
            ...
            <div className="player-console direction-panel">
                ...
                />
                <p>
                    {currentTime}ms / {durationMs}ms
                </p>
                <div className="button-row">
                    <button onClick={() => setIsPlaying((value) => !value)} type="button">
                        {isPlaying ? 'Pause' : 'Play'}
                    </button>
                    <button onClick={() => setCurrentTime((time) => Math.min(durationMs, time + 1000))} type="button">
                        +1s
                    </button>
                </div>
                ...
                <div className="runtime-list">
                    ...
                    <ol>
                        {activeAudioClips.map((clip, index) => (
                            <li key={`${clip.src ?? 'clip'}-${index}`}>{clip.src ?? clip.url ?? clip.rawSrc}</li>
                        ))}
                    </ol>
                </div>
            </div>
