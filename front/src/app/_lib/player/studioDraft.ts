...
export function applyTimelineItemTimings(
  draft: PlayerDraft,
...
export async function saveTimelineItemTimings({
  ...
  const draftResponse = await fetchImpl(
    `${apiBaseUrl}/episodes/${episodeId}/player-draft`,
  );
  ...
  const draft = draftResponse.ok
    ? ((await draftResponse.json()) as PlayerDraft)
    : structuredClone(fallbackDraft!);
