...
|-----|------|----------|--------|
| `GET /player/manifest/:episodeId` | registered draft | returns manifest with `durationMs`, `tracks`, `items`, `cues`, `media`, `records`, `tts` | covered |
| `GET /player/manifest/:episodeId` | unknown episode id | returns `404` with Nest default error body | pending |
| `GET /episodes/:episodeId/player-draft` | registered draft | returns independent draft source | covered |
